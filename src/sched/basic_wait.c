    ...
    while (1) {
        ...
        for (i = 0; i < num_pools; i++) {
            ...
            if (unit != ABT_UNIT_NULL) {
                ABTI_xstream_run_unit(&p_local_xstream, p_xstream, unit, p_pool);
                run_cnt_nowait++;
                break;
            }
        }
        ...
        if (!run_cnt_nowait) {
            ...
            if (unit != ABT_UNIT_NULL) {
                ABTI_xstream_run_unit(&p_local_xstream, p_xstream, unit,
        ...
        if (!run_cnt_nowait || (++work_count >= event_freq)) {
            ...
            ABT_bool stop =
                ABTI_sched_has_to_stop(&p_local_xstream, p_sched, p_xstream);
            if (stop == ABT_TRUE)
                break;
            work_count = 0;
        }
