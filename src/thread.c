                      ...
                      ABT_thread_attr attr, ABT_thread *newthread)
{
    ...
    ABTI_thread_type unit_type =
        (newthread != NULL)
            ? (ABTI_THREAD_TYPE_THREAD_USER | ABTI_THREAD_TYPE_NAMED)
            : ABTI_THREAD_TYPE_THREAD_USER;
    abt_errno =
        ...
        for (i = 0; i < num; i++) {
            ...
            abt_errno =
                ABTI_thread_create_internal(p_local_xstream, p_pool, thread_f,
                                            ...
                                            ABTI_THREAD_TYPE_THREAD_USER, NULL,
...
int ABT_thread_free(ABT_thread *thread)
{
    ...
    ABTI_CHECK_TRUE_MSG(p_local_xstream == NULL ||
                            &p_thread->thread != p_local_xstream->p_thread,
                        ABT_ERR_INV_THREAD,
                        "The current thread cannot be freed.");

    ABTI_CHECK_TRUE_MSG(ABTI_thread_type_is_thread_user(p_thread->thread.type),
                        ABT_ERR_INV_THREAD,
...
int ABT_thread_exit(void)
{
    ...
    /* Terminate this ULT */
    ABTD_thread_exit(p_local_xstream, p_ythread);
...
int ABT_thread_cancel(ABT_thread thread)
{
    ...
    ABTI_thread_set_request(&p_thread->thread, ABTI_THREAD_REQ_CANCEL);
...
int ABT_thread_self(ABT_thread *thread)
{
    ...
    if (ABTI_thread_type_is_thread(p_self->type)) {
        *thread = ABTI_thread_get_handle(ABTI_thread_get_ythread(p_self));
    } else {
...
int ABT_thread_get_state(ABT_thread thread, ABT_thread_state *state)
{
    ...
    *state = ABTI_thread_state_get_thread_state(
        (ABTI_thread_state)ABTD_atomic_acquire_load_int(
            &p_thread->thread.state));
...
int ABT_thread_get_last_pool(ABT_thread thread, ABT_pool *pool)
{
    ...
    /* Return value */
    *pool = ABTI_pool_get_handle(p_thread->thread.p_pool);
...
int ABT_thread_get_last_pool_id(ABT_thread thread, int *id)
{
    ...
    ABTI_ASSERT(p_thread->thread.p_pool);
    *id = (int)(p_thread->thread.p_pool->id);
...
int ABT_thread_yield_to(ABT_thread thread)
{
    ...
    p_cur_thread = ABTI_thread_get_ythread(p_local_xstream->p_thread);
#else
    ...
    ABTI_CHECK_TRUE_MSG(p_cur_thread->thread.p_pool ==
                            p_tar_thread->thread.p_pool,
                        ABT_ERR_INV_THREAD,
    ...
    ABTD_atomic_release_store_int(&p_cur_thread->thread.state,
    ...
    ABTI_POOL_REMOVE(p_tar_thread->thread.p_pool, p_tar_thread->thread.unit,
                     ABTI_self_get_native_thread_id(p_local_xstream));

    /* We set the last ES */
    p_tar_thread->thread.p_last_xstream = p_local_xstream;

    /* Switch the context */
    ABTD_atomic_release_store_int(&p_tar_thread->thread.state,
...
int ABT_thread_migrate_to_sched(ABT_thread thread, ABT_sched sched)
{
    ...
    ABTI_CHECK_TRUE(ABTI_thread_type_is_thread_user(p_thread->thread.type),
                    ABT_ERR_INV_THREAD);
    ABTI_CHECK_TRUE(ABTD_atomic_acquire_load_int(&p_thread->thread.state) !=
                        ABTI_THREAD_STATE_TERMINATED,
                    ABT_ERR_INV_THREAD);

    /* Find a pool */
    ABTI_pool *p_pool;
    ABTI_sched_get_migration_pool(p_sched, p_thread->thread.p_pool, &p_pool);
    ABTI_CHECK_NULL_POOL_PTR(p_pool);

    abt_errno = ABTI_thread_migrate_to_pool(&p_local_xstream, &p_thread->thread,
                                            p_pool);
    ABTI_CHECK_ERROR(abt_errno);
...
int ABT_thread_migrate(ABT_thread thread)
{
    ...
    /* TODO: choose a pool also when (p_thread->thread.p_pool->consumer ==
     * NULL) */
    while (1) {
        /* Only one ES */
        ...
        if (p_xstream && p_xstream != p_thread->thread.p_last_xstream) {
                            ...
                            void *cb_arg)
{
    ...
    ABTI_thread_mig_data *p_mig_data =
        ABTI_thread_get_mig_data(p_local_xstream, &p_thread->thread);
...
int ABT_thread_set_migratable(ABT_thread thread, ABT_bool flag)
{
    ...
    if (ABTI_thread_type_is_thread_user(p_thread->thread.type)) {
        if (flag) {
            p_thread->thread.type |= ABTI_THREAD_TYPE_MIGRATABLE;
        } else {
            p_thread->thread.type &= ~ABTI_THREAD_TYPE_MIGRATABLE;
        }
    }
...
int ABT_thread_is_migratable(ABT_thread thread, ABT_bool *flag)
{
    ...
    *flag = (p_thread->thread.type & ABTI_THREAD_TYPE_MIGRATABLE) ? ABT_TRUE
                                                                  : ABT_FALSE;
...
int ABT_thread_is_primary(ABT_thread thread, ABT_bool *flag)
{
    ...
    *flag = ABTI_thread_type_is_thread_main(p_thread->thread.type) ? ABT_TRUE
                                                                   : ABT_FALSE;
...
int ABT_thread_is_unnamed(ABT_thread thread, ABT_bool *flag)
{
    ...
    *flag =
        (p_thread->thread.type & ABTI_THREAD_TYPE_NAMED) ? ABT_FALSE : ABT_TRUE;
...
int ABT_thread_set_specific(ABT_thread thread, ABT_key key, void *value)
{
    ...
    /* Set the value. */
    ABTI_ktable_set(p_local_xstream, &p_thread->thread.p_keytable, p_key,
                    value);
...
int ABT_thread_get_specific(ABT_thread thread, ABT_key key, void **value)
{
    ...
    /* Get the value. */
    *value = ABTI_ktable_get(&p_thread->thread.p_keytable, p_key);
fn_exit:
    return abt_errno;
...
int ABT_thread_get_attr(ABT_thread thread, ABT_thread_attr *attr)
{
    ...
    thread_attr.migratable =
        (p_thread->thread.type & ABTI_THREAD_TYPE_MIGRATABLE) ? ABT_TRUE
                                                              : ABT_FALSE;
    ABTI_thread_mig_data *p_mig_data =
        (ABTI_thread_mig_data *)ABTI_ktable_get(&p_thread->thread.p_keytable,
                            ...
                            ABTI_ythread **pp_newthread)
{
    ...
    if ((ABTI_thread_type_is_thread_main(thread_type) ||
         ABTI_thread_type_is_thread_main_sched(thread_type)) &&
    ...
    if (p_sched && ABTI_thread_type_is_thread_user(thread_type)) {
    ...
    } else if (ABTI_thread_type_is_thread_main_sched(thread_type)) {
                                ...
                                ABTI_thread *p_thread, ABTI_pool *p_pool)
{
    ...
    ABTI_CHECK_TRUE(ABTI_pool_accept_migration(p_pool, p_thread->p_pool) ==
                        ABT_TRUE,
                    ABT_ERR_INV_POOL);
    ABTI_CHECK_TRUE(ABTI_thread_type_is_thread_user(p_thread->type),
                    ABT_ERR_INV_THREAD);
    ...
    if (p_local_xstream != NULL && p_thread == p_local_xstream->p_thread &&
        ABTI_thread_type_is_thread(p_thread->type)) {
        ABTI_ythread *p_ythread = ABTI_thread_get_ythread(p_thread);
        ABTI_thread_yield(pp_local_xstream, p_ythread,
                            ...
                            ABTI_xstream *p_xstream, ABTI_ythread **p_thread)
{
    ...
    abt_errno =
        ABTI_thread_create_internal(p_local_xstream, p_pool, NULL, NULL, &attr,
                                    ABTI_THREAD_TYPE_THREAD_MAIN, NULL,
    ...
    if (p_xstream->type == ABTI_XSTREAM_TYPE_PRIMARY) {
        ...
        abt_errno =
            ABTI_thread_create_internal(p_local_xstream, NULL,
                                        ...
                                        (void *)p_xstream, &attr,
                                        ABTI_THREAD_TYPE_THREAD_MAIN_SCHED,
                             ...
                             ABTI_sched *p_sched)
{
    ...
    abt_errno =
        ABTI_thread_create_internal(p_local_xstream, p_pool,
                                    ...
                                    &attr, ABTI_THREAD_TYPE_THREAD_USER,
                                    p_sched, NULL, ABT_TRUE,
                                    &p_sched->p_thread);
    ABTI_CHECK_ERROR(abt_errno);
...
int ABTI_thread_set_blocked(ABTI_ythread *p_thread)
{
    ...
    ABTI_thread_set_request(&p_thread->thread, ABTI_THREAD_REQ_BLOCK);
...
{
    ABTI_xstream *p_local_xstream = *pp_local_xstream;
    ABTI_ASSERT(&p_thread->thread == p_local_xstream->p_thread);
...
int ABTI_thread_set_ready(ABTI_xstream *p_local_xstream, ABTI_ythread *p_thread)
{
    ...
    while (ABTD_atomic_acquire_load_uint32(&p_thread->thread.request) &
     ...
     * p_thread->thread.p_pool by ABT_unit_set_associated_pool. */
    ABTI_pool *p_pool = p_thread->thread.p_pool;
...
static inline ABT_bool ABTI_thread_is_ready(ABTI_ythread *p_thread)
{
    ...
    ABTI_pool *p_pool = p_thread->thread.p_pool;
    if (p_pool->u_is_in_pool(p_thread->thread.unit) == ABT_TRUE &&
        ABTD_atomic_acquire_load_int(&p_thread->thread.state) ==
            ABTI_THREAD_STATE_READY) {
        return ABT_TRUE;
    }
...
void ABTI_thread_print(ABTI_ythread *p_thread, FILE *p_os, int indent)
{
    ...
    ABTI_xstream *p_xstream = p_thread->thread.p_last_xstream;
    int xstream_rank = p_xstream ? p_xstream->rank : 0;
    char *type, *state;

    if (ABTI_thread_type_is_thread_main(p_thread->thread.type)) {
        type = "MAIN";
    } else if (ABTI_thread_type_is_thread_main_sched(p_thread->thread.type)) {
        type = "MAIN_SCHED";
    } else if (ABTI_thread_type_is_thread_user(p_thread->thread.type)) {
        type = "USER";
    } else {
        type = "UNKNOWN";
    }
    switch (ABTD_atomic_acquire_load_int(&p_thread->thread.state)) {
                              ...
                              ABTI_ythread *p_thread)
{
    ...
    ABTI_CHECK_TRUE(ABTD_atomic_relaxed_load_int(&p_thread->thread.state) ==
                        ABTI_THREAD_STATE_TERMINATED,
                    ABT_ERR_INV_THREAD);
    ...
    p_thread->thread.f_thread = thread_func;
    p_thread->thread.p_arg = arg;

    ABTD_atomic_relaxed_store_int(&p_thread->thread.state,
                                  ABTI_THREAD_STATE_READY);
    ABTD_atomic_relaxed_store_uint32(&p_thread->thread.request, 0);
    p_thread->thread.p_last_xstream = NULL;
    p_thread->thread.p_parent = NULL;

    if (p_thread->thread.p_pool != p_pool) {
        ...
        p_thread->thread.unit = p_pool->u_create_from_thread(h_thread);
    }
...
#else
    abt_errno = ABTI_pool_push(p_pool, p_thread->thread.unit,
                               ABTI_self_get_native_thread_id(p_local_xstream));
    ABTI_CHECK_ERROR(abt_errno);
#endif
                                   ...
                                   ABTI_ythread *p_thread)
{
    ...
    if (ABTD_atomic_acquire_load_int(&p_thread->thread.state) ==
        ABTI_THREAD_STATE_TERMINATED) {
        goto fn_exit;
    }
    ...
    ABTI_CHECK_TRUE_MSG(&p_thread->thread != p_self_thread, ABT_ERR_INV_THREAD,
                        "The target ULT should be different.");
    ABTI_ythread *p_self = ABTI_thread_get_ythread(p_self_thread);
    ABT_pool_access access = p_self->thread.p_pool->access;

    if ((p_self->thread.p_pool == p_thread->thread.p_pool) &&
        ...
        while (p_thread->thread.p_pool->u_is_in_pool(p_thread->thread.unit) !=
               ABT_TRUE) {
        }
        ...
        /* Remove the target ULT from the pool */
        ABTI_POOL_REMOVE(p_thread->thread.p_pool, p_thread->thread.unit,
                         ABTI_self_get_native_thread_id(p_local_xstream));
        ...
        ABTD_atomic_relaxed_store_thread_context_ptr(&p_thread->ctx.p_link,
                                                     &p_self->ctx);
        /* Set the last ES */
        p_thread->thread.p_last_xstream = p_xstream;
        ABTD_atomic_release_store_int(&p_thread->thread.state,
                                      ABTI_THREAD_STATE_RUNNING);
        ...
        uint32_t req = ABTD_atomic_fetch_or_uint32(&p_thread->thread.request,
...
yield_based:
    while (ABTD_atomic_acquire_load_int(&p_thread->thread.state) !=
           ABTI_THREAD_STATE_TERMINATED) {
        ABTI_thread_yield(pp_local_xstream,
                          ABTI_thread_get_ythread(p_self_thread),
                          ABT_SYNC_EVENT_TYPE_THREAD_JOIN, (void *)p_thread);
        p_local_xstream = *pp_local_xstream;
    }
    goto fn_exit;
