                      ...
                      ABT_thread_attr attr, ABT_thread *newthread)
{
    ...
    abt_errno =
        ABTI_thread_create_internal(p_local_xstream, p_pool, thread_func, arg,
                                    ...
                                    refcount, NULL, ABT_TRUE, &p_newthread);
...
int ABT_thread_free(ABT_thread *thread)
{
    ...
    ABTI_CHECK_TRUE_MSG(p_thread->type != ABTI_THREAD_TYPE_THREAD_MAIN &&
                            p_thread->type !=
...
int ABT_thread_yield_to(ABT_thread thread)
{
    ...
    ABTI_thread *p_prev =
        ABTI_thread_context_switch_to_sibling(&p_local_xstream, p_cur_thread,
                                              p_tar_thread);
    ABTI_tool_event_thread_run(p_local_xstream, p_cur_thread, p_prev,
                               p_cur_thread->p_parent);
...
int ABT_thread_migrate_to_sched(ABT_thread thread, ABT_sched sched)
{
    ...
    ABTI_CHECK_TRUE(p_thread->type != ABTI_THREAD_TYPE_THREAD_MAIN &&
                        p_thread->type != ABTI_THREAD_TYPE_THREAD_MAIN_SCHED,
                    ABT_ERR_INV_THREAD);
...
int ABT_thread_migrate(ABT_thread thread)
{
    ...
    /* TODO: choose a pool also when (p_thread->p_pool->consumer ==
     * NULL) */
    while (1) {
        /* Only one ES */
...
int ABT_thread_set_migratable(ABT_thread thread, ABT_bool flag)
{
    ...
    if (p_thread->type == ABTI_THREAD_TYPE_THREAD_USER) {
...
int ABT_thread_is_primary(ABT_thread thread, ABT_bool *flag)
{
    ...
    *flag =
        (p_thread->type == ABTI_THREAD_TYPE_THREAD_MAIN) ? ABT_TRUE : ABT_FALSE;
...
int ABT_thread_is_unnamed(ABT_thread thread, ABT_bool *flag)
{
    ...
    *flag = (p_thread->refcount == 0) ? ABT_TRUE : ABT_FALSE;
...
int ABT_thread_set_specific(ABT_thread thread, ABT_key key, void *value)
{
    ...
    /* Set the value. */
    ABTI_thread_set_specific(p_local_xstream, p_thread, p_key, value);
fn_exit:
    return abt_errno;
...
int ABT_thread_get_specific(ABT_thread thread, ABT_key key, void **value)
{
    ...
    /* Get the value. */
    *value = ABTI_thread_get_specific(p_thread, p_key);
fn_exit:
    return abt_errno;
...
int ABT_thread_get_attr(ABT_thread thread, ABT_thread_attr *attr)
{
    ...
    thread_attr.p_stack = p_thread->ctx.p_stack;
    thread_attr.stacksize = p_thread->ctx.stacksize;
    thread_attr.stacktype = p_thread->ctx.stacktype;
#ifndef ABT_CONFIG_DISABLE_MIGRATION
                       ...
                       ABTI_thread_attr *p_attr, ABTI_thread **pp_newthread)
{
    ...
    abt_errno =
        ABTI_thread_create_internal(p_local_xstream, p_pool, thread_func, arg,
                                    p_attr, ABTI_THREAD_TYPE_THREAD_USER, NULL,
                            ...
                            ABTI_xstream *p_xstream, ABTI_thread **p_thread)
{
    ...
    abt_errno =
        ABTI_thread_create_internal(p_local_xstream, p_pool, NULL, NULL, &attr,
                                    ABTI_THREAD_TYPE_THREAD_MAIN, NULL, 0,
                                  ...
                                  ABTI_xstream *p_xstream, ABTI_sched *p_sched)
{
    ...
    if (p_xstream->type == ABTI_XSTREAM_TYPE_PRIMARY) {
         ...
         * primary ULT. */
        ABTD_atomic_relaxed_store_thread_context_ptr(&p_newthread->ctx.ctx
                                                          .p_link,
...
int ABTI_thread_set_blocked(ABTI_thread *p_thread)
{
    ...
    ABTI_CHECK_TRUE(p_thread->type != ABTI_THREAD_TYPE_THREAD_MAIN_SCHED,
                    ABT_ERR_THREAD);
    ...
    ABTD_atomic_release_store_int(&p_thread->state, ABTI_THREAD_STATE_BLOCKED);
...
int ABTI_thread_set_ready(ABTI_xstream *p_local_xstream, ABTI_thread *p_thread)
{
     ...
     * p_thread->p_pool by ABT_unit_set_associated_pool. */
    ABTI_pool *p_pool = p_thread->p_pool;
...
void ABTI_thread_print(ABTI_thread *p_thread, FILE *p_os, int indent)
{
    ...
    switch (p_thread->type) {
        case ABTI_THREAD_TYPE_THREAD_MAIN:
            type = "MAIN";
            break;
        case ABTI_THREAD_TYPE_THREAD_MAIN_SCHED:
            type = "MAIN_SCHED";
            break;
                                                     ...
                                                     FILE *p_os)
{
    ...
    size_t i, j, stacksize = p_thread->ctx.stacksize;
                                   ...
                                   ABTI_thread *p_thread)
{
    ...
    if ((p_self->p_pool == p_thread->p_pool) &&
        ...
        ABTD_atomic_relaxed_store_thread_context_ptr(&p_thread->ctx.ctx.p_link,
                                                     &p_self->ctx.ctx);
        /* Set the last ES */
        ...
        ABTD_atomic_release_store_int(&p_self->state,
                                      ABTI_THREAD_STATE_BLOCKED);
        ...
        ABTI_tool_event_thread_run(p_local_xstream, p_self, p_prev,
                                   p_self->p_parent);
         ...
         * is needed here. */
        ABTD_atomic_release_store_thread_context_ptr(&p_thread->ctx.ctx.p_link,
                                                     &p_self->ctx.ctx);
