...
int ABTI_xstream_start(ABTI_xstream *p_local_xstream, ABTI_xstream *p_xstream)
{
    ...
    if (p_xstream->type == ABTI_XSTREAM_TYPE_PRIMARY) {
        ...
        abt_errno = ABTI_thread_create_main_sched(p_local_xstream, p_xstream, p_sched);
        ABTI_CHECK_ERROR(abt_errno);
 ...
 */
int ABTI_xstream_start_primary(ABTI_xstream **pp_local_xstream, ABTI_xstream *p_xstream,
                               ABTI_thread *p_thread)
{
    ...
    /* Create the main sched ULT */
    ABTI_sched *p_sched = p_xstream->p_main_sched;
    abt_errno = ABTI_thread_create_main_sched(*pp_local_xstream, p_xstream, p_sched);
    ABTI_CHECK_ERROR(abt_errno);
    p_sched->p_thread->p_last_xstream = p_xstream;
...
int ABT_xstream_free(ABT_xstream *xstream)
{
    ...
    ABTI_CHECK_TRUE_MSG(p_local_xstream == NULL || p_xstream != p_local_xstream->p_xstream,
                        ABT_ERR_INV_XSTREAM,
...
int ABT_xstream_set_main_sched(ABT_xstream xstream, ABT_sched sched)
{
    ...
    if (p_xstream->p_main_sched) {
        ...
        if (ABTI_sched_get_effective_size(p_local_xstream, p_xstream->p_main_sched) >
            0) {
    ...
    abt_errno = ABTI_xstream_update_main_sched(&p_local_xstream, p_xstream, p_sched);
    ABTI_CHECK_ERROR(abt_errno);
...
int ABT_xstream_run_unit(ABT_unit unit, ABT_pool pool)
{
    ...
    abt_errno = ABTI_xstream_run_unit(&p_local_xstream, p_xstream, unit, p_pool);
    ABTI_CHECK_ERROR(abt_errno);
...
int ABTI_xstream_join(ABTI_xstream **pp_local_xstream, ABTI_xstream *p_xstream)
{
    ...
    p_thread = p_local_xstream ? p_local_xstream->p_thread : NULL;
    if (p_thread) {
        ...
        if (access != ABT_POOL_ACCESS_MPMC) {
            ABTI_CHECK_TRUE_MSG(p_xstream != p_local_xstream->p_xstream,
                                ABT_ERR_INV_XSTREAM,
...
#endif
        ABTI_thread_context_switch_sched_to_thread(pp_local_xstream, p_sched, p_thread);
        /* The scheduler continues from here. */
        p_local_xstream = *pp_local_xstream;
                                   ...
                                   ABTI_xstream *p_xstream, ABTI_sched *p_sched)
{
    ...
    if (p_xstream->type == ABTI_XSTREAM_TYPE_PRIMARY) {
         ...
         * the new scheduler */
        abt_errno = ABTI_xstream_start_primary(pp_local_xstream, p_xstream, p_thread);
...
void *ABTI_xstream_launch_main_sched(void *p_arg)
{
    ...
    ABTI_xstream *p_local_xstream = p_xstream;
    ABTI_local_set_xstream(p_local_xstream);
    p_local_xstream->p_xstream = p_xstream;
