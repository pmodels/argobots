 ...
 * This divide-and-conquer strategy improves locality.
 *
 * ULTs cannot be simply replaced with tasklets in this case since
 * ABT_thread_free() in thread() is a synchronization operation that cannot be
 * called on a tasklet.
 */
...
void thread(void *arg)
{
    ...
    int num_pools = ((thread_arg_t *)arg)->num_pools;
    ...
    if (blockX_to - blockX_from == 1 && blockY_to - blockY_from == 1) {
        ...
        for (int ydiv = 0; ydiv < 2; ydiv++) {
            for (int xdiv = 0; xdiv < 2; xdiv++) {
                ...
                thread_args[index].num_pools = num_pools;
                if (thread_args[index].blockX_to -
                    ...
                    thread_args[index].blockY_to -
                            thread_args[index].blockY_from !=
                        0) {
                    /* Push a ULT to an arbitrary pool. */
                    int pool_index =
                        (thread_args[index].blockX_from * num_blocksY +
                         thread_args[index].blockY_from) %
                        num_pools;
                    ABT_thread_create(pools[pool_index], thread,
                                      &thread_args[index], ABT_THREAD_ATTR_NULL,
                                      &threads[index]);
                }
            }
        }
...
int main(int argc, char **argv)
{
    ...
    for (int t = 0; t < num_iters; t++) {
        ...
        thread_arg.num_pools = num_xstreams;
        thread(&thread_arg);
