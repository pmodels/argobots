...
/*
 * Base implementation: stencil_forkjoin_task.c
 * Reference: stencil_forkjoin_revive.c
 *
 * Parallel 2D stencil code based on a fork-join strategy.  In the first
 * iteration, it creates as many tasklets as the number of blocks (num_blocksX *
 * num_blocksY) and frees them in the last iteration.  For halo synchronization,
 * tasklets are joined and revived (not freed and created) between iterations.
 *
 * Tasklets are more lightweight than ULTs, but the performance is further
 * optimized by join and revive. The join operation takes a created tasklet and
 * waits for its completion, and the revive operation restarts a joined
 * tasklet.  Join and revive operations are similar to free and create but are
 * more efficient because data structures are reused.
 */
        ...
        for (int blockX = 0; blockX < num_blocksX; blockX++) {
            for (int blockY = 0; blockY < num_blocksY; blockY++) {
                ...
                if (t == 0) {
                    ...
                    ABT_task_revive(pools[pool_id], kernel, p_kernel_arg,
                                    &tasks[index]);
                }
            }
        }
